...
function Stations() {
  ...
  const getName = (s) => s?.STATION_NAME || s?.station_name || s?.name || s?.stationName || s?.id || "대여소 목록";
  ...
  const filtered = useMemo(() => {
    ...
    if (term) {
      items = items.filter((s) => {
        ...
        const addr = String(s?.ADDRESS || s?.address || s?.addr || "").toLowerCase();
      ...
      <div style={{ marginTop: 12, border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', background: '#fff' }}>
        ...
        <div>
          {pageItems.map((s, idx) => {
            ...
            const addr = s?.ADDRESS || s?.address || s?.addr || "";
