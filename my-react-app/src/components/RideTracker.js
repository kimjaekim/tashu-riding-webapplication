...
function RideTracker({ user, onUpdateUser }) {
  ...
  const getRemainingTime = () => {
    ...
    const estimatedTotal = 1800; // 30분 예상 (초 단위)
  ...
  const getRemainingDistance = () => {
    ...
    const currentDistance = currentRide.distance || 0;
    const estimatedTotal = 5.0; // 5km 예상
    const remaining = Math.max(0, estimatedTotal - currentDistance);
    return remaining;
  };
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#16a34a' }}>
                {(currentRide?.distance || 0).toFixed(2)}km
              </div>
            </div>
            <div style={{
              background: '#fef3c7',
              ...
              <div style={{ fontSize: '12px', color: '#d97706', marginBottom: '2px' }}>
                남은시간
              </div>
